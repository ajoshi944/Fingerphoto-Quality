...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_all=False):
        ...
        l2_x = self.layer2(l1_x)


        l3_x = self.layer3(l2_x)


        l4_x = self.layer4(l3_x)
...
class Fuse(nn.Module):
    def __init__(self, args):
        ...
        # self.c1 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=(1, 1)),
        #                         nn.LeakyReLU(),
        #                         nn.AdaptiveAvgPool2d((1, 1)))
        # self.r1 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=(1, 1)),
        #                         nn.LeakyReLU(),
        #                         nn.AdaptiveAvgPool2d((1, 1)))
        self.att_1 = SelfAttN(64)
        self.norm_1 = nn.LayerNorm(64)
        self.fc1 = nn.Sequential( nn.ReLU(), nn.Linear(64, 512), nn.ReLU())
        # self.c2 = nn.Sequential(nn.Conv2d(128, 1, kernel_size=(1, 1)),
        #                         nn.LeakyReLU(),
        #                         nn.AdaptiveAvgPool2d((1, 1)))
        # self.r2 = nn.Sequential(nn.Conv2d(128, 1, kernel_size=(1, 1)),
        #                         nn.LeakyReLU(),
        #                         nn.AdaptiveAvgPool2d((1, 1)))
        self.att_2 = SelfAttN(128)
        self.norm_2 = nn.LayerNorm(128)
        self.fc2 = nn.Sequential( nn.ReLU(), nn.Linear(128, 512), nn.ReLU())
        # self.c3 = nn.Sequential(nn.Conv2d(256, 1, kernel_size=(1, 1)),
        #                         nn.LeakyReLU(),
        #                         nn.AdaptiveAvgPool2d((1, 1)))
        # self.r3 = nn.Sequential(nn.Conv2d(128, 1, kernel_size=(1, 1)),
        #                         nn.LeakyReLU(),
        #                         nn.AdaptiveAvgPool2d((1, 1)))

        self.att_3 = SelfAttN(256)
        self.norm_3 = nn.LayerNorm(256)
        self.fc3 = nn.Sequential( nn.ReLU(), nn.Linear(256, 512), nn.ReLU())

        self.att_4 = SelfAttN(512)
    ...
    def forward(self, candidate_feat, reference_feat, matching_score, label):
        c_x, c_l3_x, c_l2_x, c_l1_x = candidate_feat
        r_x, r_l3_x, r_l2_x, r_l1_x = reference_feat
        # attended = self.att(candidate_feat, reference_feat)
        # attended += candidate_feat
        # x = self.mlp(attended) # bs x 256 x 8 x 8
        # c_l1_x = self.c1(c_l1_x)
        # r_l1_x = self.r1(r_l1_x)
        c_l1_x = self.avgpool(c_l1_x)
        r_l1_x = self.avgpool(r_l1_x)
        attended_x1 = self.att_1(c_l1_x, r_l1_x)
        attended_x1 = attended_x1.view(c_l1_x.shape[0], -1)
        attended_x1 = self.norm_1(attended_x1)
        attended_x1 = self.fc1(attended_x1)


        c_l2_x = self.avgpool(c_l2_x)
        r_l2_x = self.avgpool(r_l2_x)
        attended_x2 = self.att_2(c_l2_x, r_l2_x)
        attended_x2 = attended_x2.view(c_l2_x.shape[0], -1)
        attended_x2 = self.norm_2(attended_x2)
        attended_x2 = self.fc2(attended_x2)

        c_l3_x = self.avgpool(c_l3_x)
        r_l3_x = self.avgpool(r_l3_x)
        attended_x3 = self.att_3(c_l3_x, r_l3_x)
        attended_x3 = attended_x3.view(c_l3_x.shape[0], -1)
        attended_x3 = self.norm_3(attended_x3)
        attended_x3 = self.fc3(attended_x3)

        attended = self.att_4(c_x, r_x)
        x = attended.view(attended.shape[0], -1)
        x = self.norm_4(x)

        x = torch.cat([attended_x1, attended_x2, attended_x3, x], dim=1)
        # print(x.shape)
        # exit()
        score = self.mlp(x)
        # pair = self.fc2(x)
        ...
        label = (label - 0.5) / 0.5
        loss4 = self.criterion2(c_x.view(label.shape[0], -1), r_x.view(label.shape[0], -1), label) * 2.0
...
class Quality(nn.Module):
    ...
    def forward(self, x, target=None, cand_qm=None):
        ...
        if target is not None or cand_qm is not None:
            ...
            qm_loss = self.criterion(qm_res, cand_qm) * 10.
            return loss, qm_loss
        return score, qm_res
