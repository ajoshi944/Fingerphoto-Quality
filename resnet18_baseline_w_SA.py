...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=False):
        ...
        # self.q_conv = nn.Conv2d(128, 128//8, kernel_size=1)
        # self.k_conv = nn.Conv2d(128, 128 // 8, kernel_size=1)
        # self.v_conv = nn.Conv2d(128, 128, kernel_size=1)
        self.att1 = nn.MultiheadAttention(128, 1, batch_first=True)
        self.att2 = nn.MultiheadAttention(256, 1, batch_first=True)
        # self.att3 = nn.MultiheadAttention(128, 1, batch_first=True)

        # self.dense_mlp = nn.Sequential(nn.Conv2d(512, 1024, 1, 1, 0), nn.ReLU(),
        #                       nn.Conv2d(1024, 128, 1, 1, 0))
        self.split_mlps1 = SplitMLP()
    ...
    def forward(self, x, test=False):
        ...
        x = torch.permute(x.view(bs, -1, 1024), (0, 2, 1))

        x2, _ = self.att1(x, x, x)
        x2 += x
        x2 = x2.permute(0, 2, 1)
        x = x2.view(bs, -1, 32, 32)

        x = self.layer3(x)

        x = torch.permute(x.view(bs, -1, 256), (0, 2, 1))
        x2, _ = self.att2(x, x, x)
        x2 += x
        x2 = x2.permute(0, 2, 1)
        x = x2.view(bs, -1, 16, 16)
        ...
        # x = x.view(bs, -1, 64) # bs x 128 x 64
        # x = x.permute(0, 2, 1) # bs x 64 x 128
        # x2, attn_weights = self.att3(x, x, x)
        # x2 += x
        # x2 = x2.permute(0, 2, 1)
        # x = x2.view(bs, -1, 8, 8)
        ...
        # print(score_map)
        # plot_tensor([input, score_map.view(bs, 4, 4)])
        # score_map_avg = torch.mean(score_map, dim=1, keepdim=True) # bs,1


        # x = x.squeeze(1)

        # x2 = x2.reshape((bs, 128*8*8))
        # x = self.fc_big1(x2)
        # x = self.fc2(x)


        x = self.fc1(x.squeeze(1).view(bs, 64))
        x = nn.Sigmoid()(x)*100
...
class SplitMLP(nn.Module):
    ...
    def load_weights(self, mlp, mlp_name):
        checkpoint = torch.load(f'/home/n-lab/PycharmProjects/contact-quality2/results/lab2_{mlp_name}/model.pth')
...
def updated_resnet18(pretrained=False, custom_pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.load(
            f"/home/n-lab/Amol/contact-quality2/results/lab2_resnet18_reg_vin_c10_mean_manual_labeling_v3/1_model.pth")
