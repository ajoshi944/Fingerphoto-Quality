...
def plot_patches(t, rows=4, cols=5, scores=None, filename=None):
    """
...
class MLP(nn.Module):
    ...
    def __init__(self, input_size=1024, output_size=1, layers=[512, 128]):
        super().__init__()
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=False):
        ...
        # self.layer3 = self._make_layer(block, 64, layers[2], stride=1) # 256
    ...
    def forward(self, x, gt_score=None, filename=None, return_map=False):
        ...
        # print(final_score.max(), final_score.min())
        # exit()

        # plot_patches([orig, agg.view(-1, 4, 4), agg.view(-1, 4, 4)*splits[:, 0].view(-1, 32, 32), splits[:, 1].view(-1, 32, 32), splits[:, 2].view(-1, 32, 32),
...
class last_FC(nn.Module):
    ...
    def forward(self, x, return_weights=False):
        ...
        # plot_tensor([weights.view(-1, 4, 4)])
